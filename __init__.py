...
class ObjEditText(Text):
    ...
    def _update(self, dt, obj=None):  # actor._update, use obj to override self
        ...
        if self.obj:
            # track a point
            if hasattr(self.get_attrs, "__len__") and len(self.get_attrs) == 2:
                x, y = self.get_attrs[0](), self.get_attrs[1]()
                self.display_text = "%s: %0.3f, %0.3f" % (self.label, x, y)
            # display a Rect (a string attribute implies an attribute name like
            # _clickable_area
            elif type(self.set_attrs) == str:
                # calculate are we editing the x,y or the w,h
                r = getattr(self.obj, self.get_attrs, None)
                if r:
                    self.display_text = "%s: %s" % (self.label, r.serialise())
...
class Editor(object):
    ...
    def on_mouse_release(self, x, y, button, modifiers):
        """ Call the correct function depending on what the mouse has clicked on """
# x, y = x / self.game._scale, y / self.game._scale #if window is being
# scaled
        ax, ay = x, y  # asbolute x,y (for modals and menu)
        y = self.game.resolution[1] - y  # invert y-axis if needed
        ay = self.game.resolution[1] - ay
    ...
    def on_mouse_motion(self, x, y, dx, dy):
        ...
        self.game.mouse_position = x, self.game.resolution[
            1] - y  # adjusted for pyglet
        ox, oy = x, y
...
# ox, oy = ox / self.game._scale, oy / self.game._scale #if window is
# being scaled
        oy = self.game.resolution[1] - oy
    ...
    def on_mouse_drag(self, x, y, dx, dy, buttons, modifiers):
        ...
        # we are editing something so send through the new x,y in pyvida format
        if self.game._editing and self.game._editing_point_set:
            # x,y = x, self.resolution[1] - y #invert for pyglet to pyvida
            if hasattr(self.game._editing_point_get, "__len__") and len(self.game._editing_point_get) == 2:
                x, y = self.game._editing_point_get[
                    0](), self.game._editing_point_get[1]()
                x += dx
                y -= dy
                ...
                if self.game._editing_label in self.game._editing._tk_edit:
                    print("Edit", self.game._editing_label, self.game._editing._tk_edit[
                          self.game._editing_label][0])
                    self.game._editing._tk_edit[
                        self.game._editing_label][0].delete(0, 100)
                    self.game._editing._tk_edit[
                        self.game._editing_label][0].insert(0, x)

                    self.game._editing._tk_edit[
                        self.game._editing_label][1].delete(0, 100)
                    self.game._editing._tk_edit[
                        self.game._editing_label][1].insert(0, y)
...
#                    self.game._editing_point_set[1](y - self.game._editing.y)
                ...
                closest_distance = 10000.0
                r = getattr(
                    self.game._editing, self.game._editing_point_get, None)
                editing_index = None
                y = self.game.h - y
                # possible select new point
                for i, pt in enumerate([(r.left, r.top), (r.right, r.bottom)]):
                ...
                if editing_index == None:
                    return
                r2 = getattr(
                    self.game._editing, self.game._editing_point_set, None)
                if editing_index == 0:
                    r2.x += dx
                    r2.y -= dy
                else:
                    r2._w += dx
                    r2._h -= dy
                if self.game._editing_point_set == "_clickable_area":
                    self.game._editing._clickable_mask = None  # clear mask
    ...
    def object_editor(self, obj, x, y):
        ...
        for o in self._edit_object_menu:
            if o in self.game._edit_menu:
                self.game._edit_menu.remove(o)
        colour = COLOURS["cornflowerblue"]
        # TODO use self._editable?
        opt = Text(obj.name, pos=(x, y), size=size, offset=offset,
                   colour=colour, game=self.game, interact=edit_point)
...
#                tk.Checkbutton(frame, variable=self._editing_bool[label], command=toggle_bools, onvalue=True, offvalue=False).grid(row=row, column=1, columnspan=2)
            ...
            opt = ObjEditText(label, obj, get_attrs, set_attrs, format, pos=(
                x, y), size=size, offset=offset, colour=colour, game=self.game, interact=edit_fn)
            # store the colour so we can undo it after hover
            opt.colour = colour
            opt._mouse_none = option_mouse_none
            opt._mouse_motion = option_mouse_motion
        ...
        opt = Text("Edit Script", pos=(x, y), size=size, offset=offset,
                   colour=colour, game=self.game, interact=edit_script)
        self._add_object_to_editor(opt)
...
def edit_navigate(game, delta):
    ...
    if game._edit_index < 0:
        game._edit_index = num_objects - 1
    if game._edit_index >= num_objects:
        game._edit_index = 0
    obj = objects[game._edit_index]
...
def close_editor(game, btn, player):
    if game.editor.obj:
        game.editor.obj.show_debug = False
    if game._editing:
...
#    game._edit_window.on_mouse_press = e.on_mouse_press
    ...
    x = 0
    y += 40
    opt = Text("<-", pos=(x, y), size=size, offset=offset,
               game=game, interact=edit_prev)
    game._edit_menu.append(opt)

    x += 40
    opt = Text("->", pos=(x, y), size=size, offset=offset,
               game=game, interact=edit_next)
    game._edit_menu.append(opt)

    x += 40
    game._edit_menu.append(Text("New Actor", pos=(
        x, y), size=size, offset=offset, game=game, interact=new_actor))

    x += 110
    game._edit_menu.append(Text(
        "New Item", pos=(x, y), size=size, offset=offset, game=game, interact=new_item))

    x += 110
    game._edit_menu.append(Text("New Portal", pos=(
        x, y), size=size, offset=offset, game=game, interact=new_portal))

    x += 110
    game._edit_menu.append(Text("Import Object", pos=(
        x, y), size=size, offset=offset, game=game, interact=import_object))

    y += 40
    x = 0
    game._edit_menu.append(Text("Save State", pos=(
        x, y), size=size, offset=offset, game=game, interact=edit_save_state))
    x += 120
    game._edit_menu.append(Text("Load State", pos=(
        x, y), size=size, offset=offset, game=game, interact=edit_load_state))
    x += 120
    game._edit_menu.append(Text("Initial State", pos=(
        x, y), size=size, offset=offset, game=game, interact=edit_initial_state))
    x += 120
    game._edit_menu.append(Text("Close Editor", pos=(
        x, y), size=size, offset=offset, game=game, interact=close_editor))
